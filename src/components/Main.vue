<template>
  <div class="radius card" :style="{ borderRadius: 'var(--el-border-radius-round)' }">
    <div style="margin-top: 10px;margin-left: 10px;margin-right: 10px;">
      ...
      <el-button style="float: left;margin-top: -20px;margin-right: 3px" type="primary" :icon="Histogram" link
        ...
        <el-button style="float: left;margin-top: -20px;margin-left: 15px" type="primary" :icon="FullScreen" link
        @click="isFullScreen = true" />
        <el-button style="float: left;margin-top: -20px;margin-left: 15px" type="primary" link
  ...
  <AlertPanelUI
    v-model="showSchedulerPanel"
    :stats="schedulerStats"
    :schedulerConfig="scheduler.scheduler"
    :alertConfig="scheduler.alert"
    @scheduler-change="onSchedulerConfigChange"
...
import { Link, Edit, Delete, CircleCheck, Loading, CopyDocument, TrendCharts, Hide, Histogram, Calendar, FullScreen, Setting } from '@element-plus/icons-vue'
...
const showSchedulerPanel = ref(false)
const schedulerStats = ref(scheduler.getStats())

// 告警回调
scheduler.alert.onAlert = (message: string, type: 'warning' | 'error' | 'success') => {
  ElNotification({
    title: type === 'success' ? '测速完成' : '测速告警',
    message,
    type,
    duration: type === 'success' ? 8000 : 15000,
    position: 'top-right',
  })
}
...
scheduler.setCallbacks(
  () => {
    state.testTraffic = 0
    state.testSpeedSamples = []
    isRunning.value = true
  },
  () => { isRunning.value = false },
  (record) => { schedulerStats.value = scheduler.getStats() },
)
...
watch(isRunning, async (newState, oldState) => {
  ...
  if (newState) {
    ...
    // 通知调度器测速开始
    scheduler.onTestStart(state.bytesUsed)
    ...
    setUsed()
    // 通知调度器测速结束
    scheduler.onTestEnd(state.bytesUsed)
    schedulerStats.value = scheduler.getStats()
    if (!props.isVisible) {
      setTitle()
    }
  }
...
const onSchedulerConfigChange = (config: any) => {
  ...
  if (config.enabled) {
    ...
    ElMessage.success('定时测速已开启')
  } else {
    scheduler.stopScheduler()
    ElMessage.info('定时测速已关闭')
  }
}

const onAlertConfigChange = (config: any) => {
  Object.assign(scheduler.alert, config)
...
var secEvent = () => {
  ...
  // 记录本次测速速度样本
  if (speed > 0) {
    state.testSpeedSamples.push(speed)
    const samples = state.testSpeedSamples
    state.testAvgSpeed = samples.reduce((a, b) => a + b, 0) / samples.length
  }
  // 调度器 tick - 检查告警
  const shouldStop = scheduler.onTick(state.bytesUsed, speed > 0 ? speed : 0)
  if (shouldStop) {
    isRunning.value = false
    return
  }
